...
class DocDataset(data.Dataset):
    def __init__(self, path, text_field, order_field,
                 encoding='utf-8', order=None, **kwargs):
        fields = [('doc', text_field), ('order', order_field)]
        examples = []
        if order is None:
            with open(path, encoding=encoding) as f:
                for line in f:
                    ...
                    examples.append(data.Example.fromlist([line, order], fields))
        else:
            with open(path, encoding=encoding) as f, open(order) as fo:
                for line, order in zip(f, fo):
                    line = line.strip()
...
class MyBatch(Batch):
    def __init__(self, allsentences, orders, doc_len, dataset=None, device=None, oridoc=None):
        """Create a Batch from a list of examples."""
        if data is not None:
            setattr(self, 'doc_len', doc_len)
            if oridoc is not None:
                setattr(self, 'oridoc', oridoc)
    ...
    def data(self):
        ...
        for i, ex in enumerate(xs):
            if not self.sforder:
                ...
                shuffle(randid)
                ex.sfdoc = [doc[ri] for ri in randid]

                sforder = [order[ri] for ri in randid]
                ex.sforder = list(np.argsort(sforder))
    ...
    def __iter__(self):
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                for ex in minibatch:
                    doc_len.append(len(ex.sforder))

                maxdoclen = max(doc_len)

                # oridoc = []
                for ex in minibatch:
                    padnum = maxdoclen - len(ex.sforder)
                    padded = ex.sfdoc
                    for _ in range(padnum):
                        padded.append(['<pad>'])
                    allsentences.extend(padded)
                    orders.append(ex.sforder)
                    # oridoc.append(ex.doc)

                yield MyBatch(allsentences, orders, doc_len, self.dataset, self.device)
            if not self.repeat:
                return
